...
import { PeriodTableRow, PeriodTableToolbar } from '../../sections/@dashboard/period/list';
// redux
import { useDispatch, useSelector } from '../../redux/store';
import { getPeriods } from '../../redux/slices/period';
...
export default function PeriodListPage() {
  ...
  const handleEditRow = (id) => {
    navigate(PATH_DASHBOARD.period.edit(id));
  };

  const handleViewRow = (id) => {
    navigate(PATH_DASHBOARD.period.view(id));
  };
      ...
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            {
              name: 'Periods',
              href: PATH_DASHBOARD.period.root,
            },
            {
              name: 'List',
            },
              ...
              <Table size={dense ? 'small' : 'medium'} sx={{ minWidth: 800 }}>
                <TableHeadCustom
                  ...
                  onSelectAllRows={(checked) =>
                    onSelectAllRows(
                      checked,
                      tableData.map((row) => row.id_period)
                    )
                  }
                />
                  ...
                  {dataFiltered
                    ...
                    .map((row) => (
                      <PeriodTableRow
                        key={row.id_period}
                        row={row}
                        selected={selected.includes(row.id_period)}
                        onSelectRow={() => onSelectRow(row.id_period)}
                        onViewRow={() => handleViewRow(row.id_period)}
                        onEditRow={() => handleEditRow(row.id_period)}
                        onDeleteRow={() => handleDeleteRow(row.id_period)}
                      />
                    ))}
