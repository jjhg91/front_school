...
export default function ShiftListPage() {
  ...
  const handleEditRow = (id) => {
    navigate(PATH_DASHBOARD.shift.edit(id));
  };

  const handleViewRow = (id) => {
    navigate(PATH_DASHBOARD.shift.view(id));
  };
  ...
  return (
    <>
      <Helmet>
        <title> Shift: List | Minimal UI</title>
      </Helmet>
      ...
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            {
              name: 'Shifts',
              href: PATH_DASHBOARD.shift.root,
            },
            {
              name: 'List',
            },
              ...
              <Table size={dense ? 'small' : 'medium'} sx={{ minWidth: 800 }}>
                <TableHeadCustom
                  ...
                  onSelectAllRows={(checked) =>
                    onSelectAllRows(
                      checked,
                      tableData.map((row) => row.id_shift)
                    )
                  }
                />
                  ...
                  {dataFiltered
                    ...
                    .map((row) => (
                      <ShiftTableRow
                        key={row.id_shift}
                        row={row}
                        selected={selected.includes(row.id_shift)}
                        onSelectRow={() => onSelectRow(row.id_shift)}
                        onViewRow={() => handleViewRow(row.id_shift)}
                        onEditRow={() => handleEditRow(row.id_shift)}
                        onDeleteRow={() => handleDeleteRow(row.id_shift)}
                      />
                    ))}
