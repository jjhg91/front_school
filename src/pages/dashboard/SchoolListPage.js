...
import { SchoolTableRow, SchoolTableToolbar } from '../../sections/@dashboard/school/list';
// redux
...
export default function SchoolListPage() {
  ...
  return (
    <>
      <Helmet>
        <title> School: List | Minimal UI</title>
      </Helmet>
              ...
              <Table size={dense ? 'small' : 'medium'} sx={{ minWidth: 800 }}>
                <TableHeadCustom
                  ...
                  onSelectAllRows={(checked) =>
                    onSelectAllRows(
                      checked,
                      tableData.map((row) => row.id_school)
                    )
                  }
                />
                  ...
                  {dataFiltered
                    ...
                    .map((row) => (
                      <SchoolTableRow
                        key={row.id_school}
                        row={row}
                        selected={selected.includes(row.id_school)}
                        onSelectRow={() => onSelectRow(row.id_school)}
                        onViewRow={() => handleViewRow(row.id_school)}
                        onEditRow={() => handleEditRow(row.id_school)}
                        onDeleteRow={() => handleDeleteRow(row.id_school)}
                      />
                    ))}
