...
export default function CourseListPage() {
  ...
  const handleEditRow = (id) => {
    navigate(PATH_DASHBOARD.course.edit(id));
  };

  const handleViewRow = (id) => {
    navigate(PATH_DASHBOARD.course.view(id));
  };
  ...
  return (
    <>
      <Helmet>
        <title> Course: List | Minimal UI</title>
      </Helmet>
              ...
              <Table size={dense ? 'small' : 'medium'} sx={{ minWidth: 800 }}>
                <TableHeadCustom
                  ...
                  onSelectAllRows={(checked) =>
                    onSelectAllRows(
                      checked,
                      tableData.map((row) => row.id_course)
                    )
                  }
                />
                  ...
                  {dataFiltered
                    ...
                    .map((row) => (
                      <CourseTableRow
                        key={row.id_course}
                        row={row}
                        selected={selected.includes(row.id_course)}
                        onSelectRow={() => onSelectRow(row.id_course)}
                        onViewRow={() => handleViewRow(row.id_course)}
                        onEditRow={() => handleEditRow(row.id_course)}
                        onDeleteRow={() => handleDeleteRow(row.id_course)}
                      />
                    ))}
