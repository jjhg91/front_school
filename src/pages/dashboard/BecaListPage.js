...
import { BecaTableRow, BecaTableToolbar } from '../../sections/@dashboard/beca/list';
// redux
import { useDispatch, useSelector } from '../../redux/store';
import { getBecas } from '../../redux/slices/beca';
...
export default function BecaListPage() {
  ...
  const { becas } = useSelector((state) => state.beca);
  ...
  const handleEditRow = (id) => {
    navigate(PATH_DASHBOARD.beca.edit(id));
  };

  const handleViewRow = (id) => {
    navigate(PATH_DASHBOARD.beca.view(id));
  };
  ...
  return (
    <>
      <Helmet>
        <title> Beca: List | Minimal UI</title>
      </Helmet>
                  ...
                  {dataFiltered
                    ...
                    .map((row) => (
                      <BecaTableRow
                        key={row.id_beca}
                        row={row}
                        selected={selected.includes(row.id_beca)}
                        onSelectRow={() => onSelectRow(row.id_beca)}
                        onViewRow={() => handleViewRow(row.id_beca)}
                        onEditRow={() => handleEditRow(row.id_beca)}
                        onDeleteRow={() => handleDeleteRow(row.id_beca)}
                      />
                    ))}
