...
// import { updateTeacher } from '../../redux/slices/teacher';
...
export default function TeacherNewEditForm({ isEdit = false, currentTeacher,updateTeacher, createTeacher, dispatch }) {
   dayjs.extend(utc);
  ...
  const defaultValues = useMemo(
    () => ({
      cedula: currentTeacher?.User?.cedula || '',
      first_name: currentTeacher?.User?.first_name || '',
      second_name: currentTeacher?.User?.second_name || '',
      first_surname: currentTeacher?.User?.first_surname || '',
      second_surname: currentTeacher?.User?.second_surname || '',
      birth_date: currentTeacher?.User?.birth_date || '',
      nacionalidad: currentTeacher?.User?.nacionalidad || '',
      gender: currentTeacher?.User?.gender || '',
      phoneNumber_1: currentTeacher?.User?.phoneNumber_1 || '',
      phoneNumber_2: currentTeacher?.User?.phoneNumber_2 || '',
      email: currentTeacher?.User?.email || '',
      address: currentTeacher?.User?.address || '',
      marital_status: currentTeacher?.User?.marital_status || '',
      photoUrl: currentTeacher?.User?.photoUrl || '',
    }),
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if(isEdit){
          dispatch(updateTeacher(currentTeacher.id_teacher,data))
      }else{
